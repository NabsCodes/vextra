      ...
      <div className="px-6 md:px-12 lg:px-16">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm tracking-wide text-white/60">
              {homePageContent.whatWeBuild.eyebrow}
            </span>
          </div>
        </motion.div>
        ...
        >
          <h2 className="font-display text-3xl leading-[1.05] font-semibold tracking-tight md:text-5xl">
            {homePageContent.whatWeBuild.title}
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-white/65">
            {homePageContent.whatWeBuild.description}
          </p>
        </motion.div>

        <div className="grid gap-5 lg:grid-cols-3">
          {homePageContent.whatWeBuild.offerings.map((offering, index) => (
            <motion.div
              key={offering.title}
